...
async function fetchChannels(id: number): Promise<GroupChannel[]> {
  ...
  if (USE_MOCKS.groups) return mockRead(() => chatTable.groupChannels(id), { emptyValue: [] })
...
export function useJoinGroup() {
  return useGroupMembershipMutation(
    ({ id }) => (USE_MOCKS.groups ? mockWrite(() => groupsTable.join(id)) : apiFetch(`/groups/${id}/members`, groupDetailSchema, { method: 'POST' })),
    'Willkommen in der Gruppe! 🪂',
  )
}
export function useLeaveGroup() {
  return useGroupMembershipMutation(
    ({ id }) => (USE_MOCKS.groups ? mockWrite(() => groupsTable.leave(id)) : apiFetch(`/groups/${id}/members`, groupDetailSchema, { method: 'DELETE' })),
    'Gruppe verlassen.',
  )
}
...
export function useCreateFeedPost(groupId: number) {
  ...
  return useMutation({
    mutationFn: (input: FeedPostCreateInput): Promise<FeedPost> =>
      USE_MOCKS.groups ? mockWrite(() => groupsTable.createPost(groupId, input)) : apiFetch(`/groups/${groupId}/feed`, feedPostSchema, { method: 'POST', body: input }),
    onSuccess: () => {
...
export function useUpdateFeedPost(groupId: number) {
  ...
  return useMutation({
    mutationFn: ({ postId, input }: { postId: number; input: FeedPostCreateInput }): Promise<FeedPost> =>
      USE_MOCKS.groups ? mockWrite(() => groupsTable.updatePost(groupId, postId, input)) : apiFetch(`/groups/${groupId}/feed/${postId}`, feedPostSchema, { method: 'PATCH', body: input }),
...
export function useTogglePinPost(groupId: number) {
  ...
  return useMutation({
    mutationFn: (postId: number): Promise<FeedPost> =>
      USE_MOCKS.groups ? mockWrite(() => groupsTable.togglePin(groupId, postId)) : apiFetch(`/groups/${groupId}/feed/${postId}/pin`, feedPostSchema, { method: 'POST' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: qk.groups.feed(groupId) }),
  })
}
...
export function useCreateGroup() {
  ...
  return useMutation({
    mutationFn: (input: GroupCreateInput): Promise<GroupDetail> =>
      USE_MOCKS.groups ? mockWrite(() => groupsTable.create(input)) : apiFetch('/groups', groupDetailSchema, { method: 'POST', body: input }),
    onSuccess: (detail) => {
      qc.setQueryData(qk.groups.detail(detail.id), detail)
      qc.invalidateQueries({ queryKey: [...qk.groups.all, 'list'] })
    },
...
export function useUpdateGroup(id: number) {
  ...
  return useMutation({
    mutationFn: (input: GroupCreateInput): Promise<GroupDetail> =>
      USE_MOCKS.groups ? mockWrite(() => groupsTable.update(id, input)) : apiFetch(`/groups/${id}`, groupDetailSchema, { method: 'PATCH', body: input }),
    onSuccess: (detail) => {
      qc.setQueryData(qk.groups.detail(detail.id), detail)
      qc.invalidateQueries({ queryKey: [...qk.groups.all, 'list'] })
...
export function useApproveRequest() {
  return useRequestDecision(
    (g, r) => (USE_MOCKS.groups ? mockWrite(() => groupsTable.approveRequest(g, r)) : apiFetch(`/groups/${g}/join-requests/${r}/approve`, joinRequestListSchema, { method: 'POST' })),
    true,
  )
}
export function useRejectRequest() {
  return useRequestDecision(
    (g, r) => (USE_MOCKS.groups ? mockWrite(() => groupsTable.rejectRequest(g, r)) : apiFetch(`/groups/${g}/join-requests/${r}/reject`, joinRequestListSchema, { method: 'POST' })),
    false,
  )
}
...
export function useCreateInvite() {
  return useInviteMutation(
    (g) => (USE_MOCKS.groups ? mockWrite(() => groupsTable.createInvite(g)) : apiFetch(`/groups/${g}/invites`, groupInviteListSchema, { method: 'POST' })),
    'Einladungslink erstellt.',
  )
}
export function useRevokeInvite() {
  return useInviteMutation(
    (g, i) => (USE_MOCKS.groups ? mockWrite(() => groupsTable.revokeInvite(g, i)) : apiFetch(`/groups/${g}/invites/${i}`, groupInviteListSchema, { method: 'DELETE' })),
...
export function useWithdrawRequest() {
  return useGroupMembershipMutation(
    ({ id }) => (USE_MOCKS.groups ? mockWrite(() => groupsTable.withdrawRequest(id)) : apiFetch(`/groups/${id}/join-requests/mine`, groupDetailSchema, { method: 'DELETE' })),
