...
import { Card, ExperienceBadge, StatusBadge } from '@/components/ui'
import type { MeetupListItem } from '@/api/schemas'
import { formatMeetupDate } from '@/lib/format'

/** Dashboard-/Übersichts-Card eines Flugtreffens (Design-System §07). Verlinkt auf die Detailseite. */
export function MeetupCard({ meetup }: { meetup: MeetupListItem }) {
  ...
  return (
    <Link to={`/flugtreffen/${meetup.id}`} className="group block h-full">
      <Card className="flex h-full flex-col overflow-hidden shadow-card transition group-hover:shadow-hover">
        ...
        <div className="flex flex-1 flex-col p-4.5">
          <span className="mb-2 w-fit whitespace-nowrap rounded-full border-[1.5px] border-base-300 px-3 py-1 text-xs font-semibold">
